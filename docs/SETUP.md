...
首次启动会编译完整 Rust 后端（含 SQLite SQLCipher），约需 15-30 分钟。

### 日常使用

1. **默认热键**: `CmdOrCtrl+Shift+Space`
2. **复制微信消息**: 手动选中并复制对方发来的消息（`Cmd+C`）
3. **触发助手**: 按热键 `Cmd+Shift+Space` → 弹出回复候选窗口
4. **选择回复**: 点击任一候选卡片旁的"复制"按钮 → 粘贴到微信发送（`Cmd+V`）

### 风格调控

- **再保守一点**: 将回复语气临时切换为正式风格，重新生成
- **再有趣一点**: 将回复语气临时切换为幽默风格，重新生成
- **重新生成**: 使用当前设置重新生成候选回复
...
| 设置项 | 说明 |
|--------|------|
| 热键 | 默认 CmdOrCtrl+Shift+Space，可录制自定义组合键 |
| AI Provider | Claude CLI 或 Codex CLI |
| 隐私模式 | 阻断网络、仅本地处理 |
| 语气 | casual / formal / humorous |
