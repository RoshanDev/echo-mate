...
实现 EchoMate 的上下文可靠性底座，让每次生成都能解释“用了什么上下文、来源是什么、时间是否可信、是否来自用户手动补充资料”。同时增加联系人资料补充能力的基础设计：用户可以录入聊天记录之外的信息，例如“齐齐 93 年、天津人、在上海工作”，EchoMate 需要调用 Claude Code 或 Codex CLI 将其归类为结构化事实，再由本地策略决定后续什么时候放进生成 prompt。
...
4. 设计并实现手动资料归类 prompt / provider schema：输入自然语言资料，输出结构化 facts。示例输入：“齐齐 93 年，天津人，在上海工作”。示例输出应能区分出生年份/年龄段、籍贯、工作城市，并标记来源为 `manual`。
